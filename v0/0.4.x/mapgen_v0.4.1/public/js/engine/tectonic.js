...
export function generatePlates(seed, count, width, height, landmass) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    plates.push({
      ...
      elevation: isLand ? 0.3 + rng.next() * 0.4 : -0.3 - rng.next() * 0.3,
      moisture: isLand ? 0.3 + rng.next() * 0.4 : 0.7 + rng.next() * 0.3,
      temperature: isLand ? 0.4 + rng.next() * 0.3 : 0.5 + rng.next() * 0.2,
      name: `Plate ${i + 1}`,
      selected: false,
    });
  }
