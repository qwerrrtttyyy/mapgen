...
Demo 通过 Cloudflare Workers 自动部署：每次 push 到 `main` 分支会触发 `.github/workflows/deploy-cloudflare.yml`，构建 `@mapgen/web` 并发布到 Cloudflare 全球 CDN。Demo 是纯前端版本，所有地图生成在浏览器 Web Worker 中完成，无需后端。
