...
export function classifyBiome(elev, temp, moist, seaLevel, snowLine) {
    if (elev <= seaLevel)
        return 0;
    if (temp < snowLine && elev > 0.6)
        return 1;
    if (elev > 0.7)
        return 2;
    if (temp < -0.3)
        return 3;
    if (temp < 0.1)
        return moist > 0.3 ? 4 : 5;
    if (temp < 0.35)
        return moist > 0.5 ? 6 : 5;
    if (temp < 0.55) {
        if (moist > 0.7)
            return 7;
        if (moist > 0.5)
...
/** 从 RGBA 打包纹理提取单通道。 */
export function extractChannel(tex, channel, size) {
    const arr = new Float32Array(size);
    for (let i = 0; i < size; i++)
        arr[i] = tex[i * 4 + channel];
    return arr;
}
/** 从 plateTex 通道 R 还原 plateId（乘回 plateCount）。 */
export function extractPlateId(plateTex, plateCount, size) {
    const plateId = new Float32Array(size);
    for (let i = 0; i < size; i++)
        plateId[i] = Math.round(plateTex[i * 4] * plateCount);
    return plateId;
}
/**
 ...
 */
export function packAllTextures(md, elevation, slope, ridge, ridgeMask, moisture, rainfall, temperature, tempZone, riverMask, riverWidth, riverDepth, lakes, p) {
 ...
 */
export function packClimateRiverTextures(md, moisture, rainfall, temperature, tempZone, riverMask, riverWidth, riverDepth, lakes, p) {
    const size = md.width * md.height;
    for (let i = 0; i < size; i++) {
        const i4 = i * 4;
...
export function packElevTex(md, elevation, slope, ridge, ridgeMask) {
    const size = md.width * md.height;
    for (let i = 0; i < size; i++) {
        const i4 = i * 4;
 ...
 */
export function packCurrentTex(md, vx, vy, tempDelta, speed) {
    const size = md.width * md.height;
 ...
 */
export function packIceTex(md, landIce, seaIce, glacierVx, glacierVy) {
    const size = md.width * md.height;
 ...
 */
export function packBiomeTex(md, biomeId, streamOrder) {
    const size = md.width * md.height;
 ...
 */
export function packWatershedTex(md, basinId, isDivide, streamOrder) {
    const size = md.width * md.height;
 ...
 */
export function packVolcanismTex(md, volcanoProb, calderaMask, hotspotStrength) {
    const size = md.width * md.height;
 ...
 */
export function packSeasonTex(md, summerTempDelta, winterTempDelta, summerRainDelta, winterRainDelta) {
    const size = md.width * md.height;
