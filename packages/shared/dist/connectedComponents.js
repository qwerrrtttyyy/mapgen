 ...
 */
export function labelComponents(width, height, shouldLabel, areConnected) {
 ...
 */
export function computeComponentStats(width, height, labels) {
    const stats = new Map();
    for (let y = 0; y < height; y++) {
        const row = y * width;
        for (let x = 0; x < width; x++) {
 ...
 */
export function fastFloodFill(startIdx, width, height, visited, shouldVisit, onVisit) {
    const size = width * height;
