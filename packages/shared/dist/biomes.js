 ...
 *   7. Köppen 主分类
 */
export function classifyBiomes(input) {
    ...
    for (let i = 0; i < size; i++) {
        ...
        if (elev <= seaLevel) {
            ...
            // 4. 红树林（热带+海岸+潮间带）
            else if (input.coastDist && input.coastDist[i] > 0 && input.coastDist[i] < 4 &&
                temp > 0.55 && moist > 0.6) {
                id = 25;
            }
            // 5. 盐沼湿地（温带+低海拔+海岸+高湿）
            else if (input.coastDist && input.coastDist[i] > 0 && input.coastDist[i] < 6 &&
...
function koppenClassify(temp, rain, moist) {
    // 温度阈值
    ...
    if (temp < TEMPERATE) {
        // 寒带：降水模式（冬干 Dw / 湿润 Df）
        if (moist < 0.35)
            return 15; // Dwa/Dwb 温带草原冬干
        if (temp < -0.15)
            return 18; // Dfd 寒带大陆性
        if (temp < 0.0)
            return 17; // Dfc/Dfb 寒带针叶林
...
export function biomeNormalize(id) {
    return id * INV31;
}
/** 由 biomeId 获取元数据 */
export function getBiomeInfo(id) {
