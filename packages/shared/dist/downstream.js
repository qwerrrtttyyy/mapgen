 ...
 */
export function runDownstreamPipeline(input) {
    ...
    const climate = computeClimate(width, height, elevation, seaLevel, input.tempOffset, input.snowLine, input.windDirX, input.windDirY, input.rainStrength, enhance);
    ...
    const riverResult = generateRivers(width, height, elevationAfter, climate.moisture, seaLevel, input.riverCount, input.seed);
    const regions = analyzeRegions(width, height, elevationAfter, climate.moisture, climate.temperature, input.plateId, seaLevel, input.seed);
    // ── v2 新增阶段 ──
...
export function applyDownstreamToMapData(md, result, seed) {
