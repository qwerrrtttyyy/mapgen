 ...
 * 平坦区：找最低邻居（若全平则流向为 0）
 */
function computeFlowDirection(width, height, elevation, seaLevel, lakeMask) {
 ...
 */
function partitionBasins(width, height, elevation, seaLevel, flowDir, minArea) {
    const size = width * height;
    ...
    const basinOutlets = [];
    const queue = [];
    const basinOfOutlet = new Array(outlets.length);
    for (let i = 0; i < outlets.length; i++) {
        const idx = outlets[i];
        if (basinId[idx] !== -1)
            continue; // 已被其他出口 BFS 到达
        const id = basinCount++;
        basinId[idx] = id;
        basinOfOutlet[i] = id;
 ...
 */
function computeStrahlerOrder(width, height, flowDir, riverMask, seaLevel, elevation) {
    const size = width * height;
    const order = new Uint8Array(size);
    if (!riverMask)
        return order;
    // 仅河道像素参与计算
    const isRiver = (i) => riverMask[i] > 0.2 && elevation[i] > seaLevel;
...
function markDivides(width, height, basinId, elevation, seaLevel) {
    const size = width * height;
...
/** 流域分析主入口 */
export function computeWatershed(input) {
