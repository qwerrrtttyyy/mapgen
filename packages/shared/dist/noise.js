...
export class NoiseEngine {
    ...
    value2(x, y) {
        ...
        const A = p[X] + Y;
        const B = p[X + 1] + Y;
        return lerp(lerp(p[A] / 255, p[B] / 255, u), lerp(p[A + 1] / 255, p[B + 1] / 255, u), v) * 2 - 1;
    }
    _worleyDistances(x, y) {
    ...
    fbm(x, y, octaves, lacunarity, persistence, type = 'standard') {
        let total = 0;
        let amplitude = 1;
        let frequency = 1;
        let maxValue = 0;
     ...
     */
    fbmNatural(x, y, octaves, lacunarity, persistence, type = 'standard', opts) {
...
export function createNoise(seed, type = 'perlin') {
