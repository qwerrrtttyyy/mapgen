...
const TYPE_BASIN = 8;
const NUM_TYPES = 9;
function classifyRegionTypeId(elev, moist, temp) {
    if (elev > 0.7)
    ...
    if (temp < 0.2)
        return TYPE_TUNDRA;
    if (moist < 0.2 && temp > 0.5)
        return TYPE_DESERT;
    if (moist > 0.7 && temp > 0.4)
        return TYPE_WETLAND;
    if (moist > 0.5 && temp > 0.3)
        return TYPE_FOREST;
    return TYPE_PLAIN;
}
const TYPE_NAMES = ['mountain', 'plateau', 'hill', 'tundra', 'desert', 'wetland', 'forest', 'plain', 'basin'];
function classifyRegionType(elev, moist, temp) {
    return TYPE_NAMES[classifyRegionTypeId(elev, moist, temp)];
}
export function analyzeRegions(width, height, elevation, moisture, temperature, plateId, seaLevel, _seed) {
    ...
    for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x++) {
            const idx = row + x;
            if (visited[idx] || elevation[idx] <= seaLevel)
                continue;
...
export function computeClimate(width, height, elevation, seaLevel, tempOffset, snowLine, windDirectionX = 1, windDirectionY = 0, rainStrength = 1, enhance) {
    const size = width * height;
    ...
    // ── 行星级增强（可选）──
    if (enhance) {
        applyClimateEnhancements(width, height, elevation, seaLevel, temperature, tempZone, moisture, rainfall, enhance);
    }
 ...
 */
function applyClimateEnhancements(width, height, elevation, seaLevel, temperature, tempZone, moisture, rainfall, enhance) {
    const size = width * height;
    const invH = 1 / height;
