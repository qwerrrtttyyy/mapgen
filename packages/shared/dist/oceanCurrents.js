 ...
 */
export function computeOceanCurrents(input) {
    ...
    for (let i = 0; i < size; i++) {
        if (elevation[i] <= seaLevel)
            continue; // 陆地
        if (coastDist[i] <= 0 || coastDist[i] > COASTAL_RANGE)
            continue;
        ...
        if (cnt > 0)
            landTempDelta[i] = (sum / cnt) * (1 - coastDist[i] / COASTAL_RANGE);
    }
    // 合并：陆地为扩散值，海洋为原始值
    for (let i = 0; i < size; i++) {
        if (elevation[i] > seaLevel)
            tempDelta[i] = landTempDelta[i];
    }
