...
import type { NameablePlate, NameableRegion, NameManifest } from './naming.js';
import type { VolcanoSite, Hotspot } from './volcanism.js';
import type { NoiseType, FbmType } from './noise.js';

// 生成管线：全部编排逻辑收敛到 pipeline/*Stage，避免 index.ts 内联重复。
import { runTectonicStage, type TectonicState } from './pipeline/tectonicStage.js';
import { runElevationStage, type ElevationState } from './pipeline/elevationStage.js';
...
export interface MapParams {
  ...
  /** 取消信号。设置 `{aborted: true}` 后生成管线会在阶段边界尽早中止而非跑完整个过程。 */
...
export function generateMap(params: MapParams, onProgress?: ProgressCallback): { mapData: MapData; checkpoints: Record<string, unknown> } {
  ...
  if (params.mapWidth && params.mapHeight) {
    width = Math.min(Math.max(Math.round(params.mapWidth), 16), MAX_RESOLUTION);
    height = Math.min(Math.max(Math.round(params.mapHeight), 16), MAX_RESOLUTION);
  } else {
    const aspect = ASPECT_MAP[params.mapAspect || '1:1'] || 1;
    width = Math.min(Math.max(Math.round(params.mapSize || 512), 16), MAX_RESOLUTION);
    height = Math.round(width / aspect);
  }
  params = {
    ...params,
    plateCount: Math.min(Math.max(Math.round(params.plateCount), 2), 64),
    octaves: Math.min(Math.max(Math.round(params.octaves), 1), 12),
    erosionIterations: Math.min(Math.max(Math.round(params.erosionIterations), 0), 2000),
    riverCount: params.riverCount !== undefined ? Math.min(Math.max(Math.round(params.riverCount), 0), 20000) : undefined,
    seedStr: params.seedStr || 'default',
  };

  // 进度阶段权重（与 pipeline 各阶段对应，仅用于进度上报）。
  const phases = [
  ...
  // ── 阶段编排：全部委托给 pipeline/*Stage，消除 index.ts 内联重复 ──
  // 每阶段前检查 cancelSignal，确保取消请求尽早生效。
  advance('tectonic');
  if (params.cancelSignal?.aborted) throw new Error('Cancelled');
  const tectonic: TectonicState = runTectonicStage(width, height, seed, params);

  advance('elevation');
  if (params.cancelSignal?.aborted) throw new Error('Cancelled');
  const elevationState: ElevationState = runElevationStage(width, height, seed, params, tectonic);
  advance('erosion'); // 侵蚀已在 runElevationStage 内完成

  if (params.cancelSignal?.aborted) throw new Error('Cancelled');
  advance('coastline');
  advance('currents');
  advance('climate');
  // 在 runClimateStage 前保存高程副本，因为冰盖侵蚀会就地改写 elevationState.elevation。
  // 检查点应保存冰盖前的高程，避免编辑器增量重生成时重复应用冰盖侵蚀。
  const preIceElevation = new Float32Array(elevationState.elevation);
  ...
  const checkpointElevation = {
    elevation: new Float32Array(elevationState.elevationPre),
  ...
  const checkpointErosion = { elevation: preIceElevation };
