...
describe('流域分析', () => {
  ...
  it('流向：陆地像素都有流向（非零），海洋为零', () => {
    ...
    let landCount = 0, landWithFlow = 0;
    for (let i = 0; i < W * H; i++) {
      if (elevation[i] > 0) {
        landCount++;
  ...
  it('单一山峰 → 至少 1 个排水盆地', () => {
    ...
    const { basinCount, basinId } = computeWatershed({
      width: W, height: H, elevation, seaLevel: 0, minBasinArea: 1,
    });
  ...
  it('Strahler 河序在 [0,7] 范围内', () => {
    ...
    const { streamOrder } = computeWatershed({
      width: W, height: H, elevation, seaLevel: 0, riverMask,
    });
    for (let i = 0; i < W * H; i++) {
  ...
  it('两个分隔的山峰应产生多个盆地', () => {
    ...
    const { basinCount } = computeWatershed({
      width: W, height: H, elevation: e, seaLevel: 0, minBasinArea: 1,
    });
