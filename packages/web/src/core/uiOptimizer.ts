...
export function bindNumberArray<S extends Record<string, unknown>>(
  ...
  if (els.length === 0) {
    console.warn(`[bindNumberArray] 未找到任何元素: ${selectors}`);
    return () => {};
  }
...
export default { createStore, bindText, bindNumber, bindCheckbox, bindString, bindNumberArray, bindClass, bindAttr, bindDisabled, throttle, debounce, batchDOMWrite, renderTemplate };