...
const GENERATION_TRIGGER_CHECKBOXES = new Set<string>([
  ...
  'enableIceSheet',
  'enableMonsoon',
  'enableContinentality',
  'enableHadleyEnhancement',
]);
      ...
      .forEach(input => {
        const handler = (): void => {
          ...
          if (input.id === 'pointLightPosX' || input.id === 'pointLightPosY') {
            const x = parseFloat(
              (byId('pointLightPosX') as HTMLInputElement | null)?.value ?? '0.5'
            );
            const y = parseFloat(
              (byId('pointLightPosY') as HTMLInputElement | null)?.value ?? '0.5'
            );
